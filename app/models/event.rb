class Event < ApplicationRecord
  ...
  def self.events_by_date(type,**args)
    load_events({page: args[:page], per_page: args[:per_page]})
      .where(events:{class_date: Event.new.set_range(type,args[:year] || 2017,args[:per_page])})
  end

  def self.events_by_date_and_branch(type,**args)
    events_by_date(type,{page:args[:page],per_page:args[:per_page],year: args[:year],month: args[:month]})
      .search_by_branch_id(args[:branch])
  end

  def self.events_by_type_event(type_event,**args)
  ...
  def self.events_by_type_event_date(type_event,**args)
    events_by_type_event(type_event,{page: args[:page],per_page: args[:per_page]})
      .events_by_date({type: args[type],year: args[:year],month: args[:month],page: args[:page],month: args[:month]})
  end
